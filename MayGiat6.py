#6. Hiển thị danh sách các hóa đơn có thành tiền từ x đến y triệu đồng
...
while True:
    ...
    while True:
        try:
            y = int(input("Nhập thành tiền y (y > x): "))
            if y <= 0 or y <= x:
    ...
    write_log(f"[{timestamp}]: Danh sách các hóa đơn có thành tiền từ {x} đến {y} triệu đồng")
    found = False
    for i in content.split("\n"):
        if i!= "":
            if x <= int(i.split(",")[-1]) <= y:
                print(i)
                write_log(i)
                found = True
    if not found:

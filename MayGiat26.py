#26. Xác định số hóa đơn và tên máy giặt có thành tiền lớn nhất đã bán trong tháng x
...
while True:
    ...
    write_log(f"[{timestamp}]: Số hóa đơn và tên máy giặt có thành tiền lớn nhất đã bán trong tháng {x}:")
    max_tt = max(int(i.split(",")[-1]) for i in content.split("\n") if i and int(i.split(",")[4]) == x)
    for i in content.split("\n"):
        if i != "":
            if int(i.split(",")[4]) == x and int(i.split(",")[-1]) == max_tt:

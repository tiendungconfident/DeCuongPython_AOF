#8. Hiển thị danh sách các hóa đơn có thành tiền dưới x và trên y
...
while True:
    ...
    write_log(f"[{timestamp}]: Danh sách các hóa đơn có thành tiền dưới {x} và trên {y} triệu đồng")
    found = False
    for i in content.split("\n"):
        if i!= "":
            if int(i.split(",")[-1]) <= x or int(i.split(",")[-1]) >= y:
                print(i)
                write_log(i)
                found = True
    if not found:

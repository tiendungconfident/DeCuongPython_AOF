#9. Hiển thị danh sách các hóa đơn có thành tiền x trđ bán trong tháng z (1-9)
...
while True:
    ...
    write_log(f"[{timestamp}]: Danh sách các hóa đơn có thành tiền {x} bán trong tháng {z}:")
    found = False
    for i in content.split("\n"):
        if i!= "":
            if int(i.split(",")[-1]) == x and int(i.split(",")[4]) == z:
                print(i)
                write_log(i)
                found = True
    if not found:
